...
import 'package:untitled8/models/tile.dart';
import 'package:uuid/uuid.dart';


class BoardCubit extends Cubit<Board> {
  BoardCubit() : super(Board.newGame(0, const [])) {
  ...
  bool endRound(RoundCubit roundCubit, NextDirectionCubit directionCubit) {
    //End round.
    _endRound();
    roundCubit.end();

    //If player moved too fast before the current animation/transition finished, start the move for the next direction
    if (directionCubit.state != null) {
      move(directionCubit.state!);
      directionCubit.clear();
      return true;
    }
    return false;
  }
